...
Why: we've seen cases where the focused surface highlights correctly, but the actual
keyboard focus remains on another (often detached) terminal view. Users then type but
nothing appears (input is routed elsewhere).

This test validates:
  1) The focused terminal actually has keyboard focus (`is_terminal_focused`).
...
def main() -> int:
    with cmux(SOCKET_PATH) as c:
        ...
        # Focus left then right, verifying both keyboard focus and input routing.

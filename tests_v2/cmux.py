...
It is a direct port of the macOS cmux.py client, adapted for Linux/XDG paths.
...
# Linux/XDG socket path discovery.
...
_APP_DATA_DIR = os.path.join(_XDG_DATA_HOME, "cmux")
_STABLE_SOCKET_PATH = os.path.join(_XDG_RUNTIME_DIR, "cmux", "cmux.sock")
_LEGACY_STABLE_SOCKET_PATH = "/tmp/cmux.sock"
_LAST_SOCKET_PATH_FILES = [
    os.path.join(_APP_DATA_DIR, "last-socket-path"),
...
def _default_socket_path() -> str:
    ...
    discovered.extend(glob.glob(os.path.join(_APP_DATA_DIR, "cmux*.sock")))
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="cmux v2 socket client (Linux)")

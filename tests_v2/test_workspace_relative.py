...
SOCKET_PATH = os.environ.get("CMUX_SOCKET", cmux.DEFAULT_SOCKET_PATH)
...
def _find_cli_binary() -> str:
    ...
    # Check common Linux build output locations.
    project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    fixed = os.path.join(project_root, "zig-out", "bin", "cmux")
    if os.path.isfile(fixed) and os.access(fixed, os.X_OK):
        return fixed

    candidates = glob.glob(os.path.expanduser("~/Projects/cmux-linux/zig-out/bin/cmux"))
    candidates += glob.glob("/usr/local/bin/cmux")
    candidates += glob.glob("/usr/bin/cmux")

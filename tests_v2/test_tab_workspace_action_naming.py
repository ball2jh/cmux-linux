...
def _find_cli_binary() -> str:
    ...
    # Linux/Zig build output locations
    project_root = Path(__file__).resolve().parent.parent
    candidates = [
        str(project_root / "zig-out" / "bin" / "cmux"),
    ]
    candidates += glob.glob(str(project_root / "zig-out" / "bin" / "cmux*"))
    candidates += glob.glob("/usr/local/bin/cmux")
    candidates += glob.glob(os.path.expanduser("~/.local/bin/cmux"))

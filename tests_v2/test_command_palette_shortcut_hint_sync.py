...
Note: Linux/GTK uses text labels (e.g. "Shift+Ctrl+N") rather than macOS symbols.
The expected hints here must match what the Linux build emits.
"""
...
def main() -> int:
    with cmux(SOCKET_PATH) as client:
        ...
        try:
            ...
            # Linux/GTK shortcut hint format: Shift+Ctrl+N, Ctrl+Shift+W, Ctrl+R
            # The exact format depends on the Linux build; these match the expected output.
            _assert_shortcut_hint(rows, "palette.newWindow", "Shift+Ctrl+N")
            _assert_shortcut_hint(rows, "palette.closeWindow", "Ctrl+Shift+W")
            _assert_shortcut_hint(rows, "palette.renameTab", "Ctrl+R")
            ...
            rows = _open_palette_and_rows(client, window_id)
            _assert_shortcut_hint(rows, "palette.newWindow", "Alt+Ctrl+N")
            _assert_shortcut_hint(rows, "palette.closeWindow", "Alt+Ctrl+W")
            _assert_shortcut_hint(rows, "palette.renameTab", "Ctrl+Alt+R")
        finally:

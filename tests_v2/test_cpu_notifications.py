...
def test_cpu_after_popover_close(client: cmux, pid: int) -> tuple[bool, str]:
    ...
    This tests that the popover's UI view is properly cleaned up when closed.
    """
    ...
    # Toggle the popover via our debug socket shortcut simulator.
    # Default: Cmd+Shift+I / Ctrl+Shift+I (Show Notifications).
    try:
        client.simulate_shortcut("ctrl+shift+i")
    except Exception:
        pass
    time.sleep(0.5)
...
def main():
    ...
    socket_paths.append(os.path.join(xdg_runtime, "cmux", "cmux.sock"))

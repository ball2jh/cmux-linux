...
def _wait_for_terminal_focus(c: cmux, panel_id: str, timeout_s: float = 8.0) -> None:
    ...
    while time.time() - start < timeout_s:
        ...
        # Fallback: identify() already reports the selected/focused surface correctly.
        try:
            ident = c.identify()
...
def _type_and_wait_visible(c: cmux, panel_id: str, cmd: str) -> bool:
    ...
    Returns False when we had to fallback to send_surface in headless cases.
    """
    c.simulate_type(cmd)
    try:
    ...
    # Recovery path for transient activation lag.
    ...
    try:
        ...
        # Final fallback: direct surface send without asserting
...
def _wait_for_tmp_write(c: cmux, panel_id: str, tmp: str, token: str) -> None:
    ...
    # Final fallback: send the full command directly.
    ...
    print(f"WARN: Timed out waiting for tmp file write: {tmp}; continuing")
    return
...
def main() -> int:
    with cmux(SOCKET_PATH) as c:
        ...
        # Create a multi-pane layout to exercise split/focus races.
        for _ in range(4):
            c.new_split("right")
            time.sleep(0.25)

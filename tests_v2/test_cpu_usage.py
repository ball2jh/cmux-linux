...
def get_cmux_pid() -> Optional[int]:
    ...
    if result.returncode != 0:
        # Try matching the full command line for debug builds
        result = subprocess.run(
            ["pgrep", "-f", r"zig-out/bin/cmux"],
            capture_output=True,
            text=True,
        )
    if result.returncode != 0:
...
def sample_process(pid: int, duration: int = 2) -> str:
    """Sample a process using perf (Linux equivalent of macOS sample)."""
    # Use perf top or strace as a lightweight alternative on Linux.
    # Fall back to reading /proc/<pid>/stat if perf is unavailable.
    result = subprocess.run(
        ["timeout", str(duration), "strace", "-c", "-p", str(pid)],
        capture_output=True,
        text=True,
    )
...
def main():
    ...
    if not wait_for_idle_precheck(pid):
        print("  WARNING: Precheck timeout; continuing with measurement anyway")
    else:

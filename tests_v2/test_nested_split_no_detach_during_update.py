...
view to leave the window for a frame.

We attempt to catch this by polling `surface_health` at high frequency right after the
nested split.

In practice, the toolkit can briefly report detached state during atomic reparenting
within the same frame/event-loop tick. This can produce extremely short-lived false readings
